...
bool Direct3D::Initialize(HWND hWnd, int width, int height) {
    //・ファクトリー作成（ビデオ グラフィックの設定の列挙や指定に使用される）
    ComPtr<IDXGIFactory> factory;
    if (FAILED(CreateDXGIFactory1(IID_PPV_ARGS(&factory)))) {
        return false;
    }

    //・デバイス生成（主にリソース作成時に使用される）
    UINT creationFlags = 0;

#ifdef _DEBUG
    //・DEBUGビルド時はDirect3Dのデバッグを有効にする（とても重い）
    ...
    if (FAILED(D3D11CreateDevice(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr, creationFlags, featureLevels, _countof(featureLevels), D3D11_SDK_VERSION, &m_device, &futureLevel, &m_deviceContext))) {
        return false;
    }
